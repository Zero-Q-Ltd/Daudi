 ...
 * decide to modify at a later dateModified.
 * @enum {string}
 * @private
 */
...
DemoAnimation.prototype.handleClick_ = function (event) {
  ...
  this.movable_.classList.remove(
    this.CssClasses_.POSITION_PREFIX + this.position_
  )
...
DemoAnimation.prototype.init = function () {
  ...
  if (this.element_) {
    if (!this.movable_) {
      console.error(
        'Was expecting to find an element with class name ' +
          this.CssClasses_.MOVABLE +
          ' inside of: ',
        this.element_
      )
      return
