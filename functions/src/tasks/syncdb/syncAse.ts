...
export function syncAse(
        ...
        if (t.ItemBasedExpenseLineDetail) {
          {
            if (
              ...
              t.ItemBasedExpenseLineDetail.ItemRef.value === fuelConfig.ik.aseId
            ) {
              ValidLineItems.push({
  ...
  return Promise.all(
    ValidLineItems.map(async item => {
      const convertedASE = covertBillToASE(
        item.bill,
        item.fueltype,
        item.index
      );
      ...
      const fetchedbatch = await directory
        .where("ase.QbId", "==", convertedASE.ase.QbId)
        .get();
      /**
    ...
    return await kpcStockCollection(omcId)
      .get()
      .then(snapshot => {
...
function covertBillToASE(
  convertedBill: Bill,
  fueltype: FuelType,
  LineitemIndex: number
): ASE {
  const ASEQty = convertedBill.Line[LineitemIndex].ItemBasedExpenseLineDetail
    .Qty
