      ...
      alladmins.map(employee => {
        ...
        return getallAdmins().then(admins => {
          allDbAdmins = admins.docs.map(ad => {
            const adm: Admin_ = ad.data() as Admin_;
            adm.Id = ad.id;
            return adm;
          });
          /**
...
function adminExists(adminEmail: string): Admin_ | null {
  if (
...
function updateAdminFiields(qbAdmin: Admin_, dbAdmin: Admin_): Admin_ {
